...
As with the other charts, the median errors are fairly consistent apart from an initial single-observation value. The general correlation between total error range and number of observations is also evident to a small extent. One interesting effect seen in this graph is that box and whisker sizes (not including outliers) appear to shift over the X axis. This effect seems to be related to similar movements in the variance of ratings over time. In other words, a *rev_year* value associated with more consistent ratings seems to yield more consistent errors.
